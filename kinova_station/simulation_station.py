...
class KinovaStation(Diagram):
    ...
    def SetupDualPegScenario(self, gripper_type="hande", arm_damping=False,
                                     peg1_position=[0.8,0,0.1], peg1_rotation=[0,np.pi/2,0],
                                     peg2_position=[.7,.1,.1], peg2_rotation=[0,np.pi/2,0]):
        """
        Set up a scenario with the robot arm, a gripper, and a single peg. 
        And connect to the Drake visualizer while we're at it.
        """
        ...
        # Add first peg
        X_peg = RigidTransform()
        X_peg.set_translation(peg1_position)
        X_peg.set_rotation(RotationMatrix(RollPitchYaw(peg1_rotation)))
        self.AddManipulandFromFile("./models/manipulands/peg.sdf", X_peg)

        # Add second peg
        X_peg2 = RigidTransform()
        X_peg2.set_translation(peg2_position)
        X_peg2.set_rotation(RotationMatrix(RollPitchYaw(peg2_rotation)))
        self.AddManipulandFromFile("./models/manipulands/peg2.sdf", X_peg2)
    ...
    def go_home(self, diagram, diagram_context, name="Home"):
        ...
        if name == "Home":
            ...
            q0 = np.array([0, np.pi/4.5, np.pi, 4.014-2*np.pi, 0, 0.9599, np.pi/2])
        else:
            raise RuntimeError("Home position name must be one of ['Home', 'Retract', 'Zero','Custom']")
